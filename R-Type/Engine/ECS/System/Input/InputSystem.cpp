...
void InputSystem::createShoot(std::size_t id, ComponentManager &componentManager, Position pos, EntityManager &entityManager, std::size_t damage)
{
    ...
    std::size_t addEntity = entityManager.addMask(
        (InfoComp::SPRITEID | InfoComp::POS | InfoComp::VEL | InfoComp::PARENT | InfoComp::PROJECTILE | InfoComp::PROJECTILE | InfoComp::SIZE | InfoComp::SPRITEAT), componentManager);
    ...
    componentManager.getComponent(typeid(SpriteAttribut)).emplaceData(addEntity, SpriteAttribut{0, {0, 0, 55, 30}, sf::Color::White,
        {(sizeProj.size / _screenSize->x * _window->getSize().x), sizeProj.size / _screenSize->y * _window->getSize().y}});
    componentManager.getComponent(typeid(Position)).emplaceData(addEntity, Position{pos.x + size.x / 2,
        (pos.y + ((size.y / 2 - (30 * sizeProj.size / 2)) / _screenSize->y * _window->getSize().y)), pos.z});
    ...
    componentManager.getComponent(typeid(Size)).emplaceData(addEntity, Size{
        (55 * sizeProj.size) / _screenSize->x * _window->getSize().x, (30 * sizeProj.size) / _screenSize->y * _window->getSize().y});
