...
void ParallaxPreload::preload(Engine &engine)
{
    ...
    componentManager.getComponent(typeid(SpriteID)).emplaceData(id, SpriteID{11, Priority::HIGH, 0, 8, true, false, 0, 0.2, 640, 0});
    ...
    componentManager.getComponent(typeid(Parallax)).emplaceData(id, Parallax{true});
    id = engine.getECS().getEntityManager().addMask((InfoComp::SPRITEID | InfoComp::POS | InfoComp::VEL | InfoComp::PARALLAX | InfoComp::SPRITEAT), componentManager);
    componentManager.getComponent(typeid(SpriteID)).emplaceData(id, SpriteID{15, Priority::HIGH, 0, 8, true, false, 0, 0.2, 640, 0});
    componentManager.getComponent(typeid(SpriteAttribut)).emplaceData(id, SpriteAttribut{0, sf::FloatRect(0, 0, 640, 360), sf::Color::White, sf::Vector2f(screenSize->x / 640 / screenSize->x * windowsSize.x, screenSize->y / 360 / screenSize->y * windowsSize.y)});
    componentManager.getComponent(typeid(Position)).emplaceData(id, Position{0, 0, 0});
    ...
    componentManager.getComponent(typeid(SpriteID)).emplaceData(id, SpriteID{12, Priority::HIGH, 0, 76, true, false, 0, 0.025, 96, 0});
...
void ParallaxPreload::preload(ComponentManager &componentManager, EntityManager &entityManager, std::shared_ptr<sf::RenderWindow> window, std::shared_ptr<sf::Vector2f> screenSize)
{
