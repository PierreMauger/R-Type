...
void eng::EnemyPreload::preload(Engine &engine)
{
    ...
    std::size_t id = engine.getECS().getEntityManager().addMask(
        (InfoComp::POS | InfoComp::VEL | InfoComp::SPRITEID | InfoComp::ENEMY | InfoComp::LIFE | InfoComp::SIZE | InfoComp::PATERN | InfoComp::SPRITEAT), engine.getECS().getComponentManager());
    ...
    engine.getECS().getComponentManager().getComponent(typeid(Position)).emplaceData(id, Position{880, randY, 0});
