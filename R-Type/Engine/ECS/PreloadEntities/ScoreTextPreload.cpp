...
void ScoreTextPreload::preload(Graphic &graphic, EntityManager &entityManager, ComponentManager &componentManager)
{
    ...
    componentManager.getComponent(typeid(Text)).emplaceData(idScore, Text{"Score: ", true, 0, {static_cast<float>(graphic.getWindow()->getSize().x - 150), 10}});

    std::size_t idKill = entityManager.addMask((InfoComp::TEXT), componentManager);
    componentManager.getComponent(typeid(Text)).emplaceData(idKill, Text{"Death: ", true, 0, {static_cast<float>(graphic.getWindow()->getSize().x - 150), 50}});
}
