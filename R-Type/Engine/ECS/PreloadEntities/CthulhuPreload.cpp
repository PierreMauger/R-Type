...
void CthulhuPreload::preload(Graphic &graphic, EntityManager &entityManager, ComponentManager &componentManager, std::size_t &syncId, sf::Vector2f pos)
{
    ...
    componentManager.getComponent(typeid(SpriteAttribut)).emplaceData(id, SpriteAttribut{0, {0, 0, 110, 162}, sf::Color::White, {size.x, size.y}, {110 / size.x, 110 / size.y + 52}});
    ...
    componentManager.getComponent(typeid(Velocity)).emplaceData(id, Velocity{(windowsSize.x / (screenSize->x / 3)), size.x, 0, size.y, size.x});
