...
void VesselPreload::preload(Graphic &graphic, EntityManager &entityManager, ComponentManager &componentManager, std::size_t &syncId)
{
    ...
    componentManager.getComponent(typeid(SpriteAttribut))
        .emplaceData(idShield, SpriteAttribut{0, {0, 0, 700, 440}, sf::Color::White, {1 / screenSize->x * windowsSize.x, 1 / screenSize->y * windowsSize.y}, {(700 / screenSize->x * windowsSize.x) / 2 - (64 * size.x) / 2, (400 / screenSize->y * windowsSize.y) / 2 - (28 * size.x) / 2}});
