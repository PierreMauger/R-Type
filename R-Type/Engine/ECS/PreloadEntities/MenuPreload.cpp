...
{
    std::size_t mask = (InfoComp::BUTTON | InfoComp::POS | InfoComp::SPRITEID | InfoComp::SPRITEAT | InfoComp::TEXT);
    ...
    engine.getECS().getComponentManager().getComponent(typeid(Text)).emplaceData(id, Text{"Quit", false, 0, {static_cast<float>(windowsSize.x / 2 - 265 / 2 + 265 / 2 - 50), static_cast<float>(windowsSize.y / 2 - 116 / 2 + 116 / 2 - 50)}});
}
