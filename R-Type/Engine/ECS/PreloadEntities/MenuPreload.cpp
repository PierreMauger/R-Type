...
void MenuPreload::preload(Graphic &graphic, EntityManager &entityManager, ComponentManager &componentManager)
{
    ...
    componentManager.getComponent(typeid(Text)).emplaceData(id, Text{"Play solo", false, 0, {static_cast<float>(windowsSize.x / 3), static_cast<float>(windowsSize.y / 2)}});
    ...
    componentManager.getComponent(typeid(Text)).emplaceData(id, Text{"", false, 0, {static_cast<float>(windowsSize.x * 17 / 20), static_cast<float>(windowsSize.y / 3)}});
    componentManager.getComponent(typeid(Size)).emplaceData(id, Size{265 / screenSize->x * windowsSize.x, 116 / screenSize->y * windowsSize.y});
    componentManager.getComponent(typeid(Scene)).emplaceData(id, Scene{SceneType::MENU});

    id = entityManager.addMask((InfoComp::TEXT | InfoComp::SCENE), componentManager);
    componentManager.getComponent(typeid(Text)).emplaceData(id, Text{"IP", false, 0, {static_cast<float>(windowsSize.x * 17 / 20), static_cast<float>(windowsSize.y / 3 - 116 / 2)}});
    componentManager.getComponent(typeid(Scene)).emplaceData(id, Scene{SceneType::MENU});
    ...
    componentManager.getComponent(typeid(Text)).emplaceData(id, Text{"", false, 0, {static_cast<float>(windowsSize.x * 2 / 3), static_cast<float>(windowsSize.y / 3)}});
    componentManager.getComponent(typeid(Size)).emplaceData(id, Size{265 / screenSize->x * windowsSize.x, 116 / screenSize->y * windowsSize.y});
    componentManager.getComponent(typeid(Scene)).emplaceData(id, Scene{SceneType::MENU});

    id = entityManager.addMask((InfoComp::TEXT | InfoComp::SCENE), componentManager);
    componentManager.getComponent(typeid(Text)).emplaceData(id, Text{"PORT", false, 0, {static_cast<float>(windowsSize.x * 2 / 3), static_cast<float>(windowsSize.y / 3 - 116 / 2)}});
    componentManager.getComponent(typeid(Scene)).emplaceData(id, Scene{SceneType::MENU});
    ...
    componentManager.getComponent(typeid(Text)).emplaceData(id, Text{"Connect", false, 0, {static_cast<float>(windowsSize.x * 2 / 3), static_cast<float>(windowsSize.y / 2)}});
    componentManager.getComponent(typeid(Size)).emplaceData(id, Size{265 / screenSize->x * windowsSize.x, 116 / screenSize->y * windowsSize.y});
