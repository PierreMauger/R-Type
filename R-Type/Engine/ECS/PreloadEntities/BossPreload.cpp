...
void BossPreload::preload(Graphic &graphic, EntityManager &entityManager, ComponentManager &componentManager, std::size_t &syncId, sf::Vector2f position)
{
    ...
    componentManager.getComponent(typeid(Position)).emplaceData(id, Position{(position.x - 200) / screenSize->x * windowsSize.x, -192 / screenSize->y * windowsSize.y, 0});
    ...
    componentManager.getComponent(typeid(Size)).emplaceData(id, Size{96 / screenSize->x * windowsSize.x, 96 / screenSize->y * windowsSize.y});
