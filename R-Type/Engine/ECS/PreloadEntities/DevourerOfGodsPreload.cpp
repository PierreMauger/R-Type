...
void DevourerPreload::preload(Graphic &graphic, EntityManager &entityManager, ComponentManager &componentManager, std::size_t &syncId, sf::Vector2f position)
{
    ...
    componentManager.getComponent(typeid(SpriteAttribut)).emplaceData(id, SpriteAttribut{0, {0, 0, 375, 311}, sf::Color::White, {(screenSize->x / (windowsSize.x / 0.5f)), (screenSize->y / (windowsSize.y / 0.5f))}});
    ...
    componentManager.getComponent(typeid(Velocity)).emplaceData(id, Velocity{(screenSize->x / (windowsSize.x / 3)) * -1, 0, 0});
    ...
    componentManager.getComponent(typeid(Size)).emplaceData(id, Size{(375 * (screenSize->x / (windowsSize.x / 0.5f))), (331 * (screenSize->x / (windowsSize.x / 0.5f)))});
